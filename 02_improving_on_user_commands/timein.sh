...
# Time zone database ref: http://www.twinsun.com/ts/tz-link.htm
...
if [ $# -eq 0 ] ; then
  ...
  if [ "$matchcnt" -gt 0 ] ; then
    # But exist if more than one file matches.
    ...
    mixedregion=$(echo $mixedregion | tr -d ' ')
    mixedzone="$(echo ${zone%${zone#?}}\
      | tr '[[:lower:]]' '[[:upper:]]')\
      $(echo ${zone#?} | tr '[[:upper:]]' '[[:lower:]]')"
    mixedzone=$(echo $mixedzone | tr -d ' ')
