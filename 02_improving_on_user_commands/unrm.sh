# 16 Working with the Removed File
#!/bin/bash
...
if [ ! -d $archivedir ] ; then
  echo "$0 failed: No deleted files directory: nothing to unrm" >&2
  exit 1
fi
...
#   in the archive.
# -d to display just the folder name, not its content
matches="$(ls -d *"$1" 2> /dev/null | wc -l)"

if [ $matches -eq 0 ] ; then
  echo "No matches for \"$1\" in the deleted file archive." >&2
  exit 1
fi
