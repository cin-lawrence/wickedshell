# 45 Tracking Set User ID Application
#!/bin/bash

# findsuid--Checks all SUID files or programs to see if they're writeable,
#   and outputs the maatches in a friendly and useful format
...
# find -perm looks at the permissions of the file: 4000 and aove
#   are stuid/setgid.
find / -type f -perm /4000 -print0 2>/dev/null | while read -d '' -r match ; do
  if [ -x "$match" ] ; then
