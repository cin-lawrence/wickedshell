...
#   (or names). Fields can be ranges (a-e) or lists separaed by commas
...
fixvars()
{
  ...
  #   Save original input as "sourceline" for error message.
...
while read min hour dom mon dow command ; do
  ...
  if [ -z "$min" -o "${min%${min#?}}" = "#" ] ; then
    # If it's a blank or the first character of the line if "#", skip it.
    continue
  fi
  ...
  # At this point, all the fields in the current lines are split out into
  ...
  for hrslice in $(echo "$hour" | sed 's/[,-]/ /g') ; do
    if ! validNum $hrslice 24 ; then
      echo "Line ${line}: Invalid hour value \"$hrslice\""
      errors=1
    fi
  done
  ...
  for domslice in $(echo "$dom" | sed 's/[,-]/ /g') ; do
    if ! validNum $domslice 31 ; then
      echo "Line ${line}: Invalid day of month value \"$domslice\""
      errors=1
    fi
  done
  ...
  for monslice in $(echo "$mon" | sed 's/[,-]/ /g') ; do
    if ! validNum $monslice 12 ; then
      if ! validMon $monslice ; then
        echo "Line ${line}: Invalid month value \"$monslice\""
        errors=1
      fi
    fi
  done
