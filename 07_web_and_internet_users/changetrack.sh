...
if [ "$2" != "-" ] ; then
  ...
  echo "From $fromaddr (Website Change Tracker)"
  echo "Subject: Website $1 Has Changed"
  echo "To: $2"
  echo ""

  lynx -s -dump $1 | \
    sed -e "s|src=\"|SRC=\"$baseurl|gi" \
    sed -e "s|href=\"|HREF=\"$baseurl|gi" \
    sed -e "s|$baseurl\/http:|http:|g"
  ) | $sendmail -t
else
  # Just showing the differences on the screen is ugly. Solutions?
